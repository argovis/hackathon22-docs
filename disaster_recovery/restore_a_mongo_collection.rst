...
- The ``mongoexport`` backups don't back up indexes by default. Recreate them from the mongo shell (below are examples, need to keep track or back up actual index plans):
    ...
    .. code:: bash
      ...
      db.profiles.createIndex({"platform_id" : -1})
      db.profiles.createIndex({"data_center" : -1})
      db.profiles.createIndex({"geolocation" : "2dsphere"})
      db.profiles.createIndex({"source_info.source" : -1})
      db.profiles.createIndex({"platform_id" : -1, "timestamp" : -1})
      db.profiles.createIndex({"timestamp" : -1, "platform_id" : -1})
      db.profiles.createIndex({"date_updated_argovis" : -1})
